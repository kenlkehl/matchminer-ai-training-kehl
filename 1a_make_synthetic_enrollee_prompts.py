...
def remote_main(args, spaces: pd.DataFrame, out_dir: Path, reasoning_parser: str):
    ...
    # Collect criteria_indices already covered by existing shards (local
    # numeric shards or prior remote shards).
    done_indices = set()
    ...
    # Build prompts upfront with deterministic per-row RNG (matching local
    # mode behavior).
...
def main():
    ...
    shard_dir = out_dir / "shards"
    # Only consider numeric (local-mode) shard files; remote-mode files use the
    # 'shard_R<idx>.csv' naming and must be excluded from this calculation.
