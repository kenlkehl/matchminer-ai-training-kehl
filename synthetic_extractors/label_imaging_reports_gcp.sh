#!/usr/bin/env bash
# Start the GCP vLLM pool and label synthetic imaging reports through it.
#
...
#   SERVERS_FILE=/tmp/mmai_synthetic_imaging_servers.json
...
#   RESUME=1
#
# Extra CLI args passed to label_imaging_reports.py can be appended after --.
...
SERVERS_FILE="${SERVERS_FILE:-/tmp/mmai_synthetic_imaging_servers.json}"
...
INPUT_PARQUET="${INPUT_PARQUET:-$DATA_DIR/synthetic_imaging.parquet}"
OUTPUT_JSONL="${OUTPUT_JSONL:-$DATA_DIR/synthetic_imaging_vllm_labeled.jsonl}"
...
label_cmd=(
  "$PYTHON_BIN" "$SCRIPT_DIR/label_imaging_reports.py"
  --input "$INPUT_PARQUET"
  --output "$OUTPUT_JSONL"
  --model "$MODEL"
