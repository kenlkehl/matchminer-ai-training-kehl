#!/usr/bin/env python
"""
Train a ModernBERT-base multi-task classification model on medical oncology clinical notes.
Supports training, validation, test inference, and eval-only modes.

Usage:
    source ~/gptoss2/bin/activate
    python train_medonc_teacher.py [--args]
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train ModernBERT medonc teacher model')
    ...
    parser.add_argument('--max_seq_length', type=int, default=6144, help='Max tokenizer sequence length')
    ...
    parser.add_argument('--output_name', type=str, default='modernbert_medonc_teacher_output.parquet',
...
class MedOncTeacherModel(nn.Module):
...
def train_model(model, train_df, val_df, args, device):
    train_dataset = MedOncDataset(train_df, args.max_seq_length)
    val_dataset = MedOncDataset(val_df, args.max_seq_length)
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(pbar):
            ...
            if losses:
                total_loss = sum(losses) / args.gradient_accumulation_steps
                total_loss.backward()
                ...
                running_total_loss += sum(l.item() for l in losses)
                n_batches += 1
...
def run_inference(model, df, args, device):
    dataset = MedOncDataset(df, args.max_seq_length)
...
def main():
    ...
    model = MedOncTeacherModel(hidden_dim=args.hidden_dim)
