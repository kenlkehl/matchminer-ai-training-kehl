...
def main():
    ...
    for row in df.itertuples(index=False):
        ...
        record = {
            "row_idx": row_idx,
            "patient_id": _clean_scalar(getattr(row, args.patient_id_col, None)) if has_patient else None,
            "document_id": (
                _clean_scalar(getattr(row, args.document_id_col, None))
                if has_doc_id else f"row{row_idx}"
            ),
            "date": _clean_scalar(getattr(row, args.date_col, None)) if has_date else None,
            "note_type": _clean_scalar(getattr(row, args.note_type_col, None)) if has_note_type else None,
            "text_chars": len(text),
            "summary": summary,
    ...
    # Coerce nullable columns to string for parquet stability
    for col in ("patient_id", "document_id", "date", "note_type"):
    ...
    error_count = int(out_df["summary"].astype(str).str.startswith("ERROR:").sum())
    empty_count = int((out_df["summary"].astype(str).str.strip() == "").sum())
