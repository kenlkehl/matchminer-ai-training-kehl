...
USER_PROMPT_TEMPLATE_PREFIX = (
    "Imagine the longitudinal clinical history for 5 patients with cancer who did not quite meet criteria to enroll on a clinical trial "
    "with the following eligibility criteria:\n"
)

USER_PROMPT_TEMPLATE_SUFFIX = (
    "For example, the patient might have not had quite the right age, cancer type, histology, biomarkers, prior treatment, comorbidites, or disease burden to be eligible.\n"
    "For each patient, generate a list of events that might have occurred along the disease trajectory before the point of trial enrollment/failure to enroll.\n"
    ...
    "CRITICAL: Do not mention screening, consideration of, or enrollment on the trial itself. That would contaminate the synthetic data and render them useless, since we will be using the synthetic data to derive clinical history models to match patients to clinical trials on which they have not yet enrolled or considered.\n"
...
def parse_args():
    ...
    p.add_argument("--output-csv", default="../data/no_phi/trial_spaces_with_negative_prompts.csv", help="Final output CSV path.")
    p.add_argument("--output-dir", default="../data/no_phi/trial_spaces_negative_prompts_shards/", help="Directory for shard outputs and logs.")
...
def main():
    ...
    spaces["this_space"] = spaces['this_space'].str.replace(r'^\s*\d+\.', '', regex=True)
