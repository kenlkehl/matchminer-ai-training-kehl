#!/usr/bin/env python3
"""Append a single row-level response to <work_dir>/responses.jsonl.

Reads the response text from stdin (keeps the full text intact, including
the final "Final score: X" line) and writes one JSON object per invocation:
  {"__row_id__": <int>, "trialcheck_llm_response": <str>}
...
def main() -> None:
    ...
    line = json.dumps(
        {"__row_id__": args.row_id, "trialcheck_llm_response": text},
        ensure_ascii=False,
    )
