...
Usage:
    ...
    python run_summarization_experiments.py \
      ...
      --models gpt-oss-20b --chunk_sizes 10000 --n_patients 2
"""
...
_GPT_DEFAULTS = MODEL_CONFIGS["gpt-oss-20b"]
...
def parse_args():
    ...
    ap.add_argument("--max_model_len", type=int, default=30000,
                    help="Max model context length for vLLM (default: 30000)")
    ap.add_argument("--max_tokens", type=int, default=10000,
                    help="Max generation tokens per prompt (default: 10000)")
...
def main():
    ...
    for model in models:
        ...
        for cs in chunk_sizes:
            ...
            print(f"    chunk_size={cs:>6}  {status}")
    print()
    ...
    for batch_idx, batch_models in enumerate(model_batches):
        ...
        for slot_idx, model in enumerate(models_to_run):
            ...
            print(f"  Starting server for {model} on GPU {server_gpu_ids} (port {server_port})...")
