#!/usr/bin/env python3
"""
Evaluate ModernBERT trial checker regression model performance.

This script evaluates the ModernBERT-based trial eligibility regression model by:
- Running inference on patient-trial pairs (sigmoid + scale to 0-5)
- Computing classification metrics (AUC, F1, precision-recall) using binarized labels
- Computing ranking metrics (MAP@K)
- Computing regression metrics (Spearman, Pearson, MAE)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate ModernBERT trial checker regression model"
    )
...
def _evaluate_classification_and_ranking(validation_set: pd.DataFrame, output_dir: Path,
    ...
    pdf_path = output_dir / f"trial_checker_{mode_label}_classification.pdf"
    eval_model(
        validation_set.prediction_score.values,
        gold_binary.values,
        pdf_path=str(pdf_path),
        title_prefix=f"Trial Checker {mode_label.replace('_', ' ').title()}"
    )
...
def evaluate_patient_centric(data_dir: Path, output_dir: Path,
    ...
    print("=" * 60)
    print("EVALUATING PATIENT-CENTRIC TRIAL CHECKER (REGRESSION)")
...
def evaluate_trial_centric(data_dir: Path, output_dir: Path,
    ...
    print("=" * 60)
    print("EVALUATING TRIAL-CENTRIC TRIAL CHECKER (REGRESSION)")
    if shard_id is not None:
        print(f"(Shard {shard_id + 1}/{num_shards})")
    print("=" * 60)
