#!/usr/bin/env python3
"""
Orchestration script for the SOC (standard of care) clinical trial matching evaluation pipeline.
...
2. spacify: Create/copy trial spaces
3. retrieval: Patient-centric + trial-centric retrieval (trialspace embedding)
4. llm_checks: Eligibility + boilerplate LLM checks via API (GPT) - self-aggregating
5. oncoreasoning: OncoReasoning LLM inference via vLLM (trial check + boilerplate)
6. baseline: Baseline evaluation using Qwen3 embedding - includes retrieval,
             eligibility checks, and aggregation (no boilerplate checks)
7. evaluation: Run model evaluations and generate PDF reports
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the SOC clinical trial matching evaluation pipeline",
...
def main():
    ...
    print(f"SOC Pipeline Configuration:")
    ...
    if "prepare" in stages_to_run:
        print("\n" + "="*70)
        print("STAGE 0: PREPARE SOC DATA")
        print("="*70)

        prepare_outputs = [
            DATA_DIR / "processed_soc_treatments.csv",
            DATA_DIR / "note_level_dataset.parquet",
        ]
        if not args.force and check_outputs_exist(prepare_outputs, "SOC data preparation"):
            pass  # Skip
        else:
            cmd = [
            ...
            ret = run_command(cmd, "SOC data preparation", args.dry_run)
            if ret != 0:
    ...
    if "spacify" in stages_to_run:
        print("\n" + "="*70)
        print("STAGE 3: SELECT TRIAL SPACES")
        print("="*70)

        spacify_outputs = [
            DATA_DIR / "trial_space_lineitems.csv",
        ]
        if not args.force and check_outputs_exist(spacify_outputs, "Trial space selection"):
            pass  # Skip
        else:
            # For SOC, we select a sample of trials from v20_public_data that were
            ...
            ret = run_command(cmd, "Trial space selection", args.dry_run)
            if ret != 0:
    ...
    if "retrieval" in stages_to_run:
        print("\n" + "="*70)
        print("STAGE 4: PATIENT-TRIAL RETRIEVAL")
        print("="*70)
        ...
        if not args.force and check_outputs_exist(retrieval_outputs, "Patient-trial retrieval"):
            ...
            ok = run_sharded_retrieval(
                retrieval_tasks, gpu_list, SOC_SCRIPTS_DIR, retrieval_common_args, args.dry_run
            )
            if not ok:
    ...
    if "llm_checks" in stages_to_run:
        print("\n" + "="*70)
        print("STAGE 5: LLM ELIGIBILITY/BOILERPLATE CHECKS")
        print("="*70)
        ...
        if not tasks_to_run:
            ...
            ok = run_sharded_checks(
                tasks_to_run, gpu_list, args.download_dir, SOC_SCRIPTS_DIR, args.dry_run
            )
            if not ok:
    ...
    if "oncoreasoning" in stages_to_run:
        print("\n" + "="*70)
        print("STAGE 6: ONCOREASONING LLM INFERENCE (vLLM)")
        print("="*70)
    ...
    if "baseline" in stages_to_run:
        print("\n" + "="*70)
        print("STAGE 8: BASELINE EVALUATION (Qwen3 embedding + eligibility checks)")
        print("="*70)
        ...
        if not args.force and check_outputs_exist(baseline_outputs, "Baseline evaluation"):
            ...
            ok = run_sharded_retrieval(
                baseline_retrieval_tasks, gpu_list, SOC_SCRIPTS_DIR,
    ...
    if "evaluation" in stages_to_run:
        print("\n" + "="*70)
        print("STAGE 9: MODEL EVALUATION")
        print("="*70)
        ...
        modernbert_tasks = [
            {
                ...
                'description': "SOC ModernBERT trial checker patient-centric",
            },
            {
                'script': "eval_modernbert_trial_checker.py",
                'mode': "trial_centric",
                'output_dir': eval_output_dir / "modernbert-trial-checker",
                'model_path': args.trial_checker_model,
                'description': "SOC ModernBERT trial checker trial-centric",
            },
            {
                ...
                'description': "SOC ModernBERT boilerplate checker patient-centric",
            },
            {
                'script': "eval_modernbert_boilerplate_checker.py",
                'mode': "trial_centric",
                'output_dir': eval_output_dir / "modernbert-boilerplate-checker",
                'model_path': args.boilerplate_checker_model,
                'description': "SOC ModernBERT boilerplate checker trial-centric",
            },
        ]
            ...
            modernbert_tasks.extend([
                {
                    ...
                    'description': "SOC ModernBERT categorical trial checker patient-centric",
                ...
                },
                {
                    ...
                    'output_dir': eval_output_dir / "modernbert-trial-checker-categorical",
                    'model_path': args.categorical_trial_checker_model,
                    'description': "SOC ModernBERT categorical trial checker trial-centric",
                    'extra_args': ["--categorical"],
                },
            ])
        ...
        other_eval_tasks = [
            # Qwen3 baseline evaluation (uses baseline_ prefix for data files)
            {
                ...
                'args': ["--mode", "patient_centric", "--data-dir", str(DATA_DIR),
                         ...
                         "--prefix", "baseline_"],
                'description': "SOC Qwen3 baseline patient-centric evaluation",
            },
            {
                'script': "eval_baseline.py",
                'args': ["--mode", "trial_centric", "--data-dir", str(DATA_DIR),
                         "--output-dir", str(eval_output_dir / "qwen3-baseline"),
                         "--prefix", "baseline_"],
                'description': "SOC Qwen3 baseline trial-centric evaluation",
            },
            ...
            # Demographic-stratified breakdown of the above metrics
            {
                ...
                'description': "SOC demographic-stratified metrics (patient-centric)",
            },
            {
                'script': "eval_demographics.py",
                'args': ["--mode", "trial_centric", "--data-dir", str(DATA_DIR),
                         "--eval-dir", str(eval_output_dir),
                         "--output-dir", str(eval_output_dir / "demographics")],
                'description': "SOC demographic-stratified metrics (trial-centric)",
            },
        ]
